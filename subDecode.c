...
void decryptSubNoKey(char *key, char *message);
...
int main()
{
    ...
    /*---Rotation Without Rotation Amount---*/
    else if (c == 3)
    {
        char strng[1000];
        for (i=0; i<1000; i++)
    ...
    /*---Substitution Without Key---*/
    else if (c == 4)
    {
        char commonKey[] = "ETAOINSHRDLCUMWFGYPBVKJXQZ";
        decryptSubNoKey(commonKey, outStrng);
    }
    /*---End Main---*/
    else
...
void decryptSubNoKey(char *key, char *message)
{
    ...
    for (i = 0; i<26; i++)
        newKey[i] = 0;
    for (i=0; i<1000; i++) //this loop counts which characters occur the most.
    {
        for (l=0; l<26; l++)
        {
            if ( message[i] == alphabet[l] )
            {
                countChar[l] = countChar[l] + 1;
                break;
            }
        }
    }
